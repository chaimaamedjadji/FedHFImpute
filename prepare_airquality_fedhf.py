...
URL_ZIP = "https://archive.ics.uci.edu/ml/machine-learning-databases/00360/AirQualityUCI.zip"
...
def zscore_train(Xtr: np.ndarray, obs_tr: np.ndarray):
    ...
    for f in range(F):
        ...
        mu[f] = col.mean()
        sd[f] = col.std() + 1e-6
    Xtrn = (Xtr - mu[None, :]) / sd[None, :]
    return Xtrn.astype(np.float32), mu, sd
...
def make_clients(Xtr: np.ndarray, n_clients: int, keep_ratio: float, seed: int):
...
def main():
    ...
    ap.add_argument("--out-dir", type=str, default="airquality_fedhf")
    ap.add_argument("--raw-dir", type=str, default="raw_airquality")
    ...
    zip_path = os.path.join(args.raw_dir, "AirQualityUCI.zip")
    if not args.offline:
        download_if_missing(URL_ZIP, zip_path)
    ...
    print(f"[OK] AirQuality prepared at {args.out_dir} with F={X.shape[1]}")
